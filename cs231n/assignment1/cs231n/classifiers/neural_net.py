...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    scores_y = scores[range(N), y]
    loss = np.sum(-scores_y + np.log(temp))/N
    ...
    #############################################################################
    pass
    L = np.exp(scores)/np.sum(np.exp(scores), axis=1, keepdims=True)
    L[range(scores.shape[0]), y] -= 1
    dW2 = (hidden_layer.T).dot(L)/N + reg*W2
    hl0=hidden_layer
    hl0[hl0 > 0] = 1
    dW1 = X.T.dot(L.dot(W2.T)*hl0)/N
    db2 = L.T.dot(np.ones(N))/N
    db1 = (L.dot(W2.T)*hl0).T.dot(np.ones(N))/N
    grads['W1'] = dW1+reg*W1
    grads['W2'] = dW2
    grads['b1'] = db1
    grads['b2'] = db2
